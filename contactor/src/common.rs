...
impl ToRedisArgs for NodeInfo {
    ...
    {
        out.write_arg(&serde_json::to_vec(self).expect("failed to serialize NodeInfo"));
    }
}
...
impl FromRedisValue for RoomInfo {
    fn from_redis_value(v: &redis::Value) -> redis::RedisResult<Self> {
        let json_str: String = redis::FromRedisValue::from_redis_value(v)?;
        serde_json::from_str(&json_str).map_err(|_| {
            RedisError::from((
...
impl ToRedisArgs for RoomInfo {
    fn write_redis_args<W>(&self, out: &mut W)
    where
        W: ?Sized + redis::RedisWrite,
    {
        out.write_arg(&serde_json::to_vec(self).expect("failed to serialize RoomInfo"));
    }
}
